...
function saveDraft(data) {
  try {
    localStorage.setItem(`${DRAFT_KEY}:${DRAFT_VERSION}`, JSON.stringify(data))
  } catch {}
}
function clearDraft() {
  try {
    localStorage.removeItem(`${DRAFT_KEY}:${DRAFT_VERSION}`)
  } catch {}
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
        ...
        {isDone ? (
          ...
          <span
            style={{
              fontSize: 10,
              fontWeight: 700,
              color: isActive ? '#fff' : 'oklch(42% 0.01 250)',
            }}
          >
            {index + 1}
          </span>
      ...
      <span
        style={{
          fontSize: 10,
          color: isActive ? 'oklch(92% 0.01 250)' : 'oklch(42% 0.01 250)',
  ...
  return (
    <div
      style={{
        flex: 1,
        height: 2,
        background: isDone ? 'oklch(70% 0.18 145)' : 'oklch(28% 0.01 250 / 0.3)',
        margin: '0 4px',
        marginBottom: 18,
...
export default function Capture() {
  ...
  const textareaMetrics = useMemo(() => {
    const metrics = measureText(
      description || '请详细描述问题（位置、现象、风险等级等）...',
      '14px Plus Jakarta Sans, system-ui, sans-serif',
      343
    )
    const minHeight = 108
    const maxHeight = 240
    const calculatedHeight = Math.min(
      maxHeight,
      Math.max(minHeight, Math.round(metrics.height * 1.5))
    )
    return {
      ...metrics,
        ...
        {!isOnline && (
          <motion.div
            ...
            style={{
              ...
              gap: 6,
              padding: '8px 12px',
              background: 'oklch(80% 0.16 85 / 0.12)',
              borderRadius: 10,
              marginBottom: 12,
              borderLeft: '3px solid oklch(80% 0.16 85)',
            }}
          >
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '16px 0 20px' }}>
        {step > 0 ? (
          <motion.button
            ...
            onClick={() => {
              if (compareView) setCompareView(false)
              else setStep(s => s - 1)
            }}
            style={{
              background: 'none',
        ...
        {step === 0 && (
          ...
          >
            <div
              ...
              style={{
                border: '2px dashed oklch(28% 0.01 250 / 0.5)',
                borderRadius: 16,
                minHeight: 320,
                display: 'flex',
                flexDirection: 'column',
              ...
              {ISSUE_TYPES.map(type => (
                <motion.button
                  ...
                  style={{
                    ...
                    padding: '12px 14px',
                    background:
                      issueType === type.value ? `${type.color}25` : 'oklch(22% 0.01 250)',
                    border: `1.5px solid ${issueType === type.value ? type.color : 'oklch(28% 0.01 250 / 0.3)'}`,
                    borderRadius: 10,
                    cursor: 'pointer',
                    color: issueType === type.value ? type.color : 'oklch(92% 0.01 250)',
                    fontFamily: '"Plus Jakarta Sans", system-ui, sans-serif',
                    fontSize: 13,
          ...
          >
              ...
              <div>
                ...
                <div
                  style={{
                    ...
                    gap: 6,
                    fontSize: 12,
                    color: location ? 'oklch(65% 0.01 250)' : 'oklch(42% 0.01 250)',
                  }}
                >
                  {locating ? (
                    <span style={{ fontSize: 12 }}>定位中...</span>
                  ) : (
                    <MapPin
                      size={13}
                      color={location ? 'oklch(70% 0.18 145)' : 'oklch(42% 0.01 250)'}
                    />
                  )}
                  {location
                    ? formatCoordinate(location.lat, location.lng)
                    : locating
                      ? ''
                      : '定位失败，将以无位置提交'}
                </div>
            ...
            <motion.button
              ...
              style={{
                ...
                gap: 8,
                padding: '14px 0',
                background: submitting ? 'oklch(28% 0.01 250)' : 'oklch(60% 0.15 250)',
                color: '#fff',
                border: 'none',
                borderRadius: 10,
                fontFamily: '"Plus Jakarta Sans", system-ui, sans-serif',
