...
import { Row, Col, Card, Progress, Typography, Tag, Button, Alert, Tooltip, Badge, Timeline } from 'antd';
...
import { AlertTriangle, Users, CheckCircle2, Clock, DollarSign, TrendingUp, Warning, Zap, Calendar, HardDrive } from 'lucide-react';
    ...
    <Row gutter={16}>
      <Col span={6}>
        <div style={{ textAlign: 'center', padding: 16, background: COLORS.bg, borderRadius: 8 }}>
          <div style={{ fontSize: 32, fontWeight: 'bold', color: COLORS.primary }}>{DEPT_STATS.totalProjects}</div>
          <div style={{ fontSize: 12, color: COLORS.textMuted }}>在建项目</div>
        </div>
      </Col>
      <Col span={6}>
        <div style={{ textAlign: 'center', padding: 16, background: COLORS.bg, borderRadius: 8 }}>
          <div style={{ fontSize: 32, fontWeight: 'bold', color: COLORS.success }}>{DEPT_STATS.deliveryRate}%</div>
          <div style={{ fontSize: 12, color: COLORS.textMuted }}>交付率</div>
        </div>
      </Col>
      <Col span={6}>
        <div style={{ textAlign: 'center', padding: 16, background: COLORS.bg, borderRadius: 8 }}>
          <div style={{ fontSize: 32, fontWeight: 'bold', color: COLORS.success }}>{DEPT_STATS.idleManpower}</div>
          <div style={{ fontSize: 12, color: COLORS.textMuted }}>闲置人力</div>
        </div>
      </Col>
      <Col span={6}>
        <div style={{ textAlign: 'center', padding: 16, background: `${COLORS.danger}10`, borderRadius: 8, border: `1px solid ${COLORS.danger}30` }}>
          <div style={{ fontSize: 14, fontWeight: 'bold', color: COLORS.danger }}>🚨 {DEPT_STATS.nextWeekRisk}</div>
  ...
  >
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={PROJECTS_PROGRESS} layout="vertical">
        ...
        <YAxis type="category" dataKey="name" tick={{ fontSize: 12, fill: COLORS.text }} width={60} />
        <RechartsTooltip />
    ...
    <div style={{ marginTop: 12 }}>
      {PROJECTS_PROGRESS.map((p, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          ...
          <Progress percent={p.progress} size="small" strokeColor={p.status === 'warning' ? COLORS.warning : COLORS.primary} style={{ flex: 1 }} />
          <Tag color={p.status === 'warning' ? 'orange' : 'blue'} style={{ marginRight: 0 }}>{p.status === 'warning' ? '预警' : '正常'}</Tag>
        </div>
      ))}
    </div>
  </Card>
    ...
    >
      ...
      <ResponsiveContainer width="100%" height={150}>
        <LineChart data={RESOURCE_CONFLICT}>
          ...
          <RechartsTooltip />
          <Line type="monotone" dataKey="projectA" stroke={COLORS.primary} strokeWidth={2} dot={{ r: 4 }} name="项目A" />
          <Line type="monotone" dataKey="projectB" stroke={COLORS.success} strokeWidth={2} dot={{ r: 4 }} name="项目B" />
          <Line type="monotone" dataKey="projectC" stroke={COLORS.danger} strokeWidth={2} dot={{ r: 4 }} name="项目C" />
          <Line type="monotone" dataKey="projectD" stroke={COLORS.warning} strokeWidth={2} dot={{ r: 4 }} name="项目D" />
        </LineChart>
      </ResponsiveContainer>
      ...
      <div style={{ marginTop: 16 }}>
        {conflictData.map((c, i) => (
          <div key={i} style={{ 
            ...
            <span style={{ fontSize: 12, color: COLORS.text }}>{c.date} · {c.content}</span>
          </div>
        ))}
      </div>
    </Card>
    ...
    <Card title="人效负载雷达" style={{ borderRadius: 12 }}>
      <ResponsiveContainer width="100%" height={200}>
        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
          ...
          <PolarRadiusAxis angle={30} domain={[0, 150]} tick={{ fontSize: 10, fill: COLORS.textMuted }} />
          <Radar name="负载率" dataKey="value" stroke={COLORS.primary} fill={COLORS.primary} fillOpacity={0.3} />
        </RadarChart>
      </ResponsiveContainer>
      <div style={{ marginTop: 12 }}>
        {MANPOWER_LOAD.map((m, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            ...
            <Progress 
              percent={m.value} 
              size="small" 
              strokeColor={m.status === 'danger' ? COLORS.danger : m.status === 'warning' ? COLORS.warning : COLORS.success}
              style={{ flex: 1 }}
            />
            <Tag color={m.status === 'danger' ? 'red' : m.status === 'warning' ? 'orange' : 'green'} style={{ marginRight: 0 }}>
    ...
    {ECO_CHANGES.map((eco, i) => (
      <div key={i} style={{ 
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 20, fontWeight: 'bold', color: eco.status === 'critical' ? COLORS.danger : COLORS.warning }}>{eco.count}</div>
          <div style={{ fontSize: 10, color: COLORS.textMuted }}>次</div>
        </div>
        {i === 0 && <Tag color="red" style={{ marginRight: 0 }}>🏆 严重拖后腿</Tag>}
      </div>
    ))}
  </Card>
);
  ...
  return (
    <div style={{ padding: 24, background: COLORS.bg, minHeight: '100vh' }}>
      <div style={{ marginBottom: 24 }}>
        <Title level={3} style={{ color: COLORS.text, margin: 0 }}>前线指挥部</Title>
