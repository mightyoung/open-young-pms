...
import { 
  LayoutDashboard, Kanban, Calendar, BarChart3, Activity, BarChart,
  FolderKanban, AlertTriangle, Shield, FileSearch, GitBranch,
  ClipboardList, FileText, PieChart,
  CheckCircle2, Clock,
  MessageCircle, BookOpen, FileText as FileTextIcon,
  Users, Settings, UserCog, Bell, Search,
  LogOut, User, Cog, Database, Printer, Clipboard, BellRing, Truck, Flag, MessageSquare
} from 'lucide-react';
...
const OPTIMIZED_MENU = [
  ...
  // 第2组：驾驶舱（高层专用）
  {
    ...
    label: '驾驶舱',
    children: [
      { key: 'strategic-cockpit', label: '司令舱', icon: <BarChart3 size={16} />, badge: null, desc: '高管专用' },
      { key: 'tactical-cockpit', label: '指挥台', icon: <BarChart size={16} />, desc: '部门长专用' },
      { key: 'operational-cockpit', label: '作战台', icon: <Activity size={16} />, desc: '项目经理专用' },
    ],
  },
  // 第3组：项目管理（核心业务）
  {
...
export default function OptimizedMenu() {
  ...
  const renderMenuItem = (item) => {
          ...
          {item.children.map(child => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ color: selectedKey === child.key ? COLORS.primary : COLORS.sidebarTextMuted }}>
                    {child.icon}
                  </span>
                  <span style={{ color: selectedKey === child.key ? COLORS.sidebarText : COLORS.sidebarTextMuted }}>
                    {child.label}
                  </span>
                </div>
                {child.badge && (
                  <Badge count={child.badge} size="small" style={{ backgroundColor: COLORS.danger }} />
                )}
              </div>
            </Menu.Item>
          ))}
    ...
    return (
      <Menu.Item key={item.key} style={{ display: 'none' }} />
    );
  };
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      ...
      >
        ...
        {/* 菜单 */}
        <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px - 60px - 64px)' }}>
          <Menu
            mode="inline"
        ...
        <div style={{
          ...
          <Avatar size={32} style={{ background: `linear-gradient(135deg, ${COLORS.primary}, #1890ff)` }}>
            项目
          </Avatar>
          {!collapsed && (
            <div style={{ flex: 1, overflow: 'hidden' }}>
              ...
              <div style={{ color: COLORS.sidebarTextMuted, fontSize: 11 }}>
                管理员
              </div>
            </div>
          )}
      ...
      <Layout style={{ marginLeft: collapsed ? 80 : 260, transition: 'margin-left 0.2s' }}>
        <Header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <Button type="text" icon={collapsed ? <LayoutDashboard size={18} /> : null} onClick={() => setCollapsed(!collapsed)} />
            <Input
        ...
        <Content style={{ padding: 24, background: COLORS.bg, minHeight: 'calc(100vh - 65px)' }}>
          <div style={{ textAlign: 'center', padding: '100px 0' }}>
            ...
            <div style={{ marginTop: 24, color: '#8c8c8c', fontSize: 13, textAlign: 'left', maxWidth: 600, margin: '24px auto', lineHeight: 1.8 }}>
              <h4 style={{ color: '#323235' }}>优化要点：</h4>
              <ul>
                <li><strong>分组精简：</strong>从 11 组减少到 6 组，更易记忆</li>
                <li><strong>层级扁平：</strong>核心入口 2 层，避免深层嵌套</li>
                <li><strong>常用优先：</strong>仪表盘、看板、随手拍等高频功能置顶</li>
                <li><strong>智能分组：</strong>AI 与知识库独立分组，突出智能化定位</li>
                <li><strong>快捷入口：</strong>搜索框 + 通知Badge + 个人设置</li>
                <li><strong>视觉层次：</strong>图标 + 标签 + 徽章多维度标识</li>
              </ul>
            </div>
          </div>
        </Content>
