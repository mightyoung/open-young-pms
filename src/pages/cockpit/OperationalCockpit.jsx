...
import { Row, Col, Card, Progress, Typography, Tag, Button, Select, Badge, List, Avatar, Input, Divider, Switch, Tooltip } from 'antd';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, ResponsiveContainer } from 'recharts';
import { Monitor, MessageSquare, FileText, Package, AlertTriangle, CheckCircle2, Clock, Send, Wifi, WifiOff, ChevronDown, Zap, User, Bell, Settings, LogOut } from 'lucide-react';
...
const MESSAGES = [
  { id: 1, user: '李师傅', time: '10:20', content: '今天到厂，发现***部件尺寸不对', type: 'warning', extracted: ['异常', '尺寸错误', 'ECO'] },
  ...
  { id: 4, user: '王师傅', time: '昨天', content: '变频器参数异常，已停机检查', type: 'danger', extracted: ['设备故障', '停机'] },
];
...
const ECO_LIST = [
  { id: 1, code: 'ECO-001', title: '图纸变更001单', project: 'J-2X项目', status: 'pending', urgency: 'high' },
  { id: 2, code: 'ECO-002', title: '物料变更002单', project: 'J-2X项目', status: 'pending', urgency: 'normal' },
];
...
const ProjectSwitcher = ({ currentProject, onChange }) => (
  <Card style={{ borderRadius: 12, marginBottom: 16, background: `linear-gradient(135deg, ${COLORS.primary}, ${COLORS.primary}dd)` }}>
    ...
    <div style={{ marginTop: 12, padding: '8px 12px', background: 'rgba(255,255,255,0.15)', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
      <AlertTriangle size={14} color="#fff" />
      <span style={{ color: '#fff', fontSize: 12 }}>严厉警告：切走下方该框，下面的全部瀑布流将秒切，绝对禁止数据串台</span>
    </div>
  </Card>
);
    ...
    <div style={{ maxHeight: 300, overflow: 'auto' }}>
      {MESSAGES.map((msg, i) => (
        <div key={msg.id} style={{ 
          ...
          background: msg.type === 'danger' ? `${COLORS.danger}08` : msg.type === 'warning' ? `${COLORS.warning}08` : 'transparent'
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            ...
            <span style={{ color: COLORS.textMuted, fontSize: 11, marginLeft: 'auto' }}>{msg.time}</span>
          </div>
          <div style={{ fontSize: 13, color: COLORS.text, marginLeft: 36, marginBottom: msg.extracted.length > 0 ? 8 : 0 }}>
            "{msg.content}"
          </div>
          {msg.extracted.length > 0 && (
            <div style={{ marginLeft: 36, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              {msg.extracted.map((tag, j) => (
                <Tag key={j} color={msg.type === 'danger' ? 'red' : 'orange'} style={{ fontSize: 10 }}>
                  {tag}
                </Tag>
              ))}
              <Tag color="blue" style={{ fontSize: 10 }}>系统提取</Tag>
            </div>
          )}
        </div>
      ))}
...
const WeeklyReportGenerator = () => (
  <Card 
    ...
    extra={<Tag icon={<Zap size={12} />} style={{ background: `${COLORS.primary}15`, border: 'none', color: COLORS.primary }}>系统生成</Tag>}
  ...
  >
    <div style={{ background: COLORS.bg, padding: 16, borderRadius: 8, marginBottom: 16 }}>
      <div style={{ marginBottom: 8 }}>
        ...
        <div style={{ fontSize: 14, color: COLORS.text, fontWeight: 500 }}>{WEEKLY_REPORT.period}</div>
      </div>
    ...
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 12, color: COLORS.textMuted, marginBottom: 8 }}>
        <CheckCircle2 size={12} style={{ marginRight: 4 }} />已完成
      </div>
    ...
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 12, color: COLORS.textMuted, marginBottom: 8 }}>
        <Clock size={12} style={{ marginRight: 4 }} />待完成
      </div>
      {WEEKLY_REPORT.pending.map((item, i) => (
        <div key={i} style={{ fontSize: 13, color: COLORS.textMuted, paddingLeft: 16, marginBottom: 4 }}>
          ○ {item}
        </div>
      ))}
    </div>
    ...
    {ECO_LIST.map((eco, i) => (
      <div key={i} style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          ...
          <Badge status={eco.status === 'pending' ? 'processing' : 'success'} text={eco.status === 'pending' ? '待签字' : '已签字'} />
        </div>
        <div style={{ fontSize: 13, color: COLORS.text, fontWeight: 500, marginBottom: 4 }}>{eco.title}</div>
  ...
  >
    {MATERIAL_STATUS.map((mat, i) => (
      <div key={i} style={{ 
        ...
        <span style={{ flex: 1, fontSize: 13, color: COLORS.text, fontWeight: 500 }}>{mat.name}</span>
        <Tag color={mat.status === 'notArrived' ? 'red' : mat.status === 'partial' ? 'orange' : 'green'} style={{ marginRight: 0 }}>
          {mat.note}
        </Tag>
      </div>
    ))}
    <div style={{ marginTop: 12, padding: 8, background: COLORS.bg, borderRadius: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
...
const QuickActions = () => (
  <Card title="快捷操作台" style={{ borderRadius: 12 }}>
    <Row gutter={[8, 8]}>
      ...
      <Col span={12}>
        <Button block type="primary" icon={<CheckCircle2 size={14} />} style={{ borderRadius: 8, height: 40 }}>
          即刻同意ECO
        </Button>
      </Col>
      <Col span={12}>
  ...
  return (
    <div style={{ padding: 24, background: COLORS.bg, minHeight: '100vh' }}>
      <div style={{ marginBottom: 24 }}>
        <Title level={3} style={{ color: COLORS.text, margin: 0 }}>单兵作战台</Title>
