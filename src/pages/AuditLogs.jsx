...
export default function AuditLogs() {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'action',
      key: 'action',
      render: a => <Tag color={ACTION_MAP[a]?.color}>{ACTION_MAP[a]?.label || a}</Tag>,
    },
    {
      title: '对象',
      dataIndex: 'entity_type',
      key: 'entity_type',
      render: e => <span style={{ color: colors.text.secondary }}>{ENTITY_MAP[e] || e}</span>,
    },
    {
      title: '名称',
      dataIndex: 'entity_name',
      key: 'entity_name',
      render: t => <span style={{ color: colors.text.primary }}>{t || '-'}</span>,
    },
    {
      title: '用户',
      dataIndex: 'username',
      key: 'username',
      render: t => <span style={{ color: colors.text.muted }}>{t}</span>,
    },
    {
      title: 'IP',
      dataIndex: 'ip_address',
      key: 'ip_address',
      render: t => <span style={{ color: colors.text.disabled, fontSize: 12 }}>{t || '-'}</span>,
    },
    {
      title: '详情',
      key: 'detail',
    ...
    <div style={{ padding: 24 }}>
      ...
      <Card style={{ background: colors.bg.card, border: '1px solid #e5e7eb' }}>
        {loading ? (
          ...
          <EmptyState type="list" title="暂无日志" style={{ marginTop: 60 }} />
        ) : (
          <Table
            dataSource={data}
            columns={columns}
      ...
      >
        {detail && (
          <Descriptions
            ...
            bordered
            styles={{
              label: { color: colors.text.muted, width: 120 },
              content: { color: colors.text.primary },
            }}
          >
            <Descriptions.Item label="时间">{detail.created_at}</Descriptions.Item>
            <Descriptions.Item label="操作">
              <Tag color={ACTION_MAP[detail.action]?.color}>{ACTION_MAP[detail.action]?.label}</Tag>
            </Descriptions.Item>
            <Descriptions.Item label="对象">
              {ENTITY_MAP[detail.entity_type]} (ID: {detail.entity_id || '-'})
            </Descriptions.Item>
