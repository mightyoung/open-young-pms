...
const MOCK_DICTIONARY = [
  {
    ...
    owner: '项目管理部',
    status: 'active',
    desc: '项目唯一标识，格式：PRJ-YYYY-NNN',
    standard: '《项目管理手册》v2.1',
  },
  {
    id: 2,
    code: 'ORG-001',
  ...
  },
  {
    ...
    owner: '项目管理部',
    status: 'active',
    desc: 'PLN规划/BID招投标/EXE执行中/SUS暂停/CMP完成/CLS关闭',
    standard: '《项目管理手册》v2.1',
  },
  {
    id: 9,
    ...
    owner: '安全管理部',
    status: 'inactive',
    desc: 'SAF安全隐患/QLT质量缺陷/ENV环境问题/EQP设备问题/OTH其他',
    standard: '《隐患管理规范》',
  },
]
...
const MOCK_ENCODING = [
  ...
  },
  {
    ...
    name: '隐患编号编码规则',
    code: 'ENC-HAZ-001',
    pattern: 'HAZ-YYYYMMDD-NNN',
    example: 'HAZ-20260330-001',
    usage: '随手拍上报时自动生成',
  },
  {
    id: 3,
    name: '合同编号编码规则',
    code: 'ENC-CTR-001',
    pattern: 'CTR-[TYPE]-[YYYY]-[NNN]',
    example: 'CTR-SUP-2026-001',
    usage: '合同签订时自动生成，含类型代码',
  },
  {
    id: 4,
    name: '报告编号编码规则',
    code: 'ENC-RPT-001',
    pattern: 'RPT-[TYPE]-[YYYYMMDD]-[NNN]',
    example: 'RPT-DLY-20260330-01',
    usage: '日/周/月报提交时自动生成',
  },
  {
    id: 5,
  ...
  },
  {
    ...
    name: '设备编号编码规则',
    code: 'ENC-EQP-001',
    pattern: '[TYPE]-[YYYY]-[NNN]',
    example: 'EQP-2026-0042',
    usage: '设备入库时自动生成',
  },
]
...
export default function DataDictionary() {
  ...
  const dictColumns = [
    ...
    },
    {
      ...
      dataIndex: 'dataType',
      key: 'dataType',
      render: v => <Text style={{ color: D.textSec, fontSize: 12 }}>{v}</Text>,
    },
    {
      title: '数据等级',
      dataIndex: 'level',
    ...
    },
    {
      ...
      dataIndex: 'owner',
      key: 'owner',
      render: v => <Text style={{ color: D.textSec, fontSize: 13 }}>{v}</Text>,
    },
    {
      title: '状态',
      dataIndex: 'status',
  ...
  const encColumns = [
    ...
    },
    {
      ...
      dataIndex: 'example',
      key: 'example',
      render: v => <Text style={{ color: D.success, fontWeight: 600, fontSize: 12 }}>{v}</Text>,
    },
    {
      title: '用途',
      dataIndex: 'usage',
  ...
  return (
    <div>
      ...
      <Modal
        title={<Text style={{ fontWeight: 700 }}>{selected ? '编辑字典' : '新建字典项'}</Text>}
        open={modalOpen}
        onCancel={() => {
          setModalOpen(false)
          form.resetFields()
        ...
        <Form form={form} layout="vertical" style={{ padding: '20px 24px' }}>
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                name="code"
                label={<Text style={{ fontSize: 13 }}>编码</Text>}
                rules={[{ required: true }]}
              >
                <Input placeholder="如 PRJ-001" style={{ borderRadius: 10 }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item
                name="name"
                label={<Text style={{ fontSize: 13 }}>名称</Text>}
                rules={[{ required: true }]}
              >
                <Input placeholder="数据项名称" style={{ borderRadius: 10 }} />
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="category" label={<Text style={{ fontSize: 13 }}>分类</Text>}>
                <Select
                  options={Object.entries(CATEGORY_MAP).map(([k, v]) => ({
                    value: k,
                    label: v.label,
            ...
            <Col span={12}>
              <Form.Item name="level" label={<Text style={{ fontSize: 13 }}>数据等级</Text>}>
                <Select
                  options={Object.entries(LEVEL_CONFIG).map(([k, v]) => ({
                    value: k,
                    label: v.label,
                  }))}
                  style={{ borderRadius: 10 }}
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="dataType" label={<Text style={{ fontSize: 13 }}>数据类型</Text>}>
                <Input placeholder="VARCHAR" style={{ borderRadius: 10 }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="length" label={<Text style={{ fontSize: 13 }}>长度</Text>}>
                <Input placeholder="20" style={{ borderRadius: 10 }} />
              </Form.Item>
            </Col>
          </Row>
          <Form.Item name="owner" label={<Text style={{ fontSize: 13 }}>责任部门</Text>}>
            <Input placeholder="责任部门" style={{ borderRadius: 10 }} />
          </Form.Item>
          <Form.Item name="desc" label={<Text style={{ fontSize: 13 }}>说明</Text>}>
            <Input.TextArea rows={2} placeholder="数据项说明..." style={{ borderRadius: 10 }} />
          </Form.Item>
        </Form>
      </Modal>
    </div>
