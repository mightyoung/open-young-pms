...
import { Card, Form, Input, Select, DatePicker, Button, Upload, Modal, message, Steps, Tag, Space, Typography, Row, Col, Divider, Radio } from 'antd';
import { Camera, MapPin, Send, Clock, User, CheckCircle2, AlertTriangle, ExclamationCircle, EnvironmentOutlined } from 'lucide-react';
...
const MOCK_REPORTS = [
  { id: 1, title: '焊接质量不合格', type: 'quality', urgency: 'important', location: 'A区3号产线', reporter: '李师傅', time: '10:30', status: 'pending', images: 2 },
  { id: 2, title: '高空作业未系安全带', type: 'safety', urgency: 'urgent', location: 'B区施工现场', reporter: '王工', time: '09:15', status: 'dispatched', images: 1 },
  { id: 3, title: '变频器参数异常', type: 'equipment', urgency: 'important', location: 'C区配电室', reporter: '张师傅', time: '昨天', status: 'processing', images: 3 },
  { id: 4, title: '废液排放异常', type: 'environment', urgency: 'normal', location: '污水处理站', reporter: '刘师傅', time: '2天前', status: 'closed', images: 1 },
];
...
const PIE_DATA = HAZARD_TYPES.map(t => ({
  name: t.label, value: MOCK_REPORTS.filter(r => r.type === t.value).length * 3 + Math.floor(Math.random() * 5),
  color: t.color,
}));
  ...
  return (
    <div style={{ padding: 24, background: COLORS.bg, minHeight: '100vh' }}>
      <div style={{ marginBottom: 24 }}>
        <Title level={3} style={{ color: COLORS.text, margin: 0 }}>随手拍上报</Title>
      ...
      <Row gutter={16}>
          ...
          <Card style={{ borderRadius: 12 }}>
            {step === 0 && (
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginTop: 16 }}>
                  {HAZARD_TYPES.map(t => (
                    <div key={t.value} style={{
                      padding: 20, borderRadius: 12, border: `2px solid ${COLORS.border}`,
                ...
                <Button type="primary" block style={{ marginTop: 24, height: 44 }} onClick={() => setStep(1)}>下一步</Button>
              </div>
            )}
            ...
            {step === 1 && (
              <Form form={form} layout="vertical">
                ...
                <Form.Item name="location" label="发生位置">
                  <Input addonAfter={<Button icon={<EnvironmentOutlined />} onClick={handleLocation}>GPS定位</Button>} placeholder="手动输入位置" />
                </Form.Item>
                {location && <Tag color="green" icon={<MapPin size={12} />}>已定位: {location.lat.toFixed(4)}, {location.lng.toFixed(4)}</Tag>}
              ...
              <div>
                  ...
                  <Row gutter={16}>
                    <Col span={12}>
                      <div style={{ marginBottom: 12 }}>
                        ...
                        <div><Tag color={getTypeInfo(form.getFieldValue('type'))?.color}>{getTypeInfo(form.getFieldValue('type'))?.label}</Tag></div>
                      </div>
                      <div style={{ marginBottom: 12 }}>
                        <Text type="secondary">紧急程度</Text>
                        <div><Tag color={getUrgencyInfo(form.getFieldValue('urgency'))?.color}>{getUrgencyInfo(form.getFieldValue('urgency'))?.label}</Tag></div>
                      </div>
                    </Col>
                    <Col span={12}>
                      <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ marginTop: 24 }}>
                  <Space>
                    ...
                    <Button type="primary" icon={<Send size={14} />} onClick={handleSubmit}>确认提交</Button>
                  </Space>
                </div>
              </div>
            )}
        ...
        <Col span={8}>
          ...
          <Card title="本月上报统计" style={{ borderRadius: 12, marginBottom: 16 }}>
            <ResponsiveContainer width="100%" height={150}>
              <AreaChart data={TREND_DATA}>
                ...
                <Area type="monotone" dataKey="value" stroke={COLORS.primary} fill={`${COLORS.primary}20`} />
              </AreaChart>
            </ResponsiveContainer>
          </Card>
          ...
          <Card title="问题类型分布" style={{ borderRadius: 12 }}>
            <ResponsiveContainer width="100%" height={150}>
              <PieChart>
                <Pie data={PIE_DATA} cx="50%" cy="50%" innerRadius={30} outerRadius={50} dataKey="value">
      ...
      <Modal open={showSuccess} onCancel={() => setShowSuccess(false)} footer={[
        <Button key="ok" type="primary" onClick={() => setShowSuccess(false)}>好的</Button>
      ]}>
