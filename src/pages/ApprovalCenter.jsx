...
const MOCK_TODO = [
  {
    id: 1,
    type: 'hazard_rectification',
    title: '3号车间配电箱隐患整改验收',
    project: 'J-2X高精线联调项目',
    applicant: '王安全',
    dept: '安环部',
    createTime: '2026-03-30 10:30',
    desc: '已对配电箱门进行修复，加装防误碰装置，附整改照片。',
  },
  {
    id: 2,
    type: 'report_submit',
  ...
  },
  {
    ...
    applicant: '张总',
    dept: '总经理室',
    createTime: '2026-03-28 14:00',
    desc: '申请启动新厂区二期建设项目，计划投资1200万元。',
  },
  {
    id: 4,
    type: 'budget_adjust',
    ...
    applicant: '张经理',
    dept: '工程部',
    createTime: '2026-03-27 09:00',
    desc: '因设计变更，申请增加预算80万元。',
  },
]
const MOCK_DONE = [
  {
  ...
  },
  {
    ...
    applicant: '张经理',
    dept: '工程部',
    createTime: '2026-02-28 18:00',
    result: 'rejected',
    rejectTime: '2026-03-01 09:00',
    approver: '李总',
    reason: '进度描述过于简略，请补充具体数据和问题分析。',
  },
]
...
const WORKFLOW_TEMPLATES = [
  {
    id: 'hazard',
    name: '隐患整改验收流',
    steps: ['项目负责人初审', '科室负责人复核', '部门领导终审'],
    types: ['hazard_rectification'],
  },
  {
    id: 'report',
    name: '报告审批流',
  ...
  },
  {
    id: 'project',
    name: '项目立项审批流',
    steps: ['部门领导初审', '公司领导复审', '总经理终审'],
    types: ['project_create'],
  },
  {
    id: 'budget',
    name: '预算调整审批流',
    steps: ['项目经理申请', '财务审核', '分管领导审批', '总经理终审'],
    types: ['budget_adjust'],
  },
]
...
export default function ApprovalCenter() {
  ...
  const handleApprove = item => {
    Modal.confirm({
      title: '确认通过',
      content: `确认通过「${item.title}」？`,
      okText: '确认通过',
      okButtonProps: { style: { background: D.success, borderRadius: 10 } },
      onOk: () => {
  ...
  const handleReject = item => {
    Modal.confirm({
      ...
      content: (
        <div>
          <Text style={{ color: D.textSec }}>请输入驳回原因：</Text>
          <Input.TextArea
            rows={2}
            value={rejectReason}
      ...
      okText: '确认驳回',
      okButtonProps: { style: { background: D.danger, borderRadius: 10 } },
      onOk: () => {
        message.success('已驳回')
        setDetailOpen(false)
        setRejectReason('')
  ...
  return (
    <div>
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 20 }}>
        {[
          { title: '待我审批', value: MOCK_TODO.length, color: D.warning },
          {
            title: '已通过',
            value: MOCK_DONE.filter(d => d.result === 'approved').length,
            color: D.success,
          },
          {
            title: '已驳回',
            value: MOCK_DONE.filter(d => d.result === 'rejected').length,
            color: D.danger,
          },
          { title: '审批流模板', value: WORKFLOW_TEMPLATES.length, color: D.primary },
        ].map((s, i) => (
        ...
        items={[
          {
            key: 'todo',
            label: (
              <Badge count={MOCK_TODO.length} size="small" offset={[8, -2]}>
                <span>待我审批</span>
              </Badge>
            ),
          },
          ...
          {WORKFLOW_TEMPLATES.map((wf, i) => (
            <Col xs={24} sm={12} key={wf.id}>
              ...
              >
                ...
                >
                  <Title level={5} style={{ color: D.text, margin: 0 }}>
                    {wf.name}
                  </Title>
                  <Button
                    type="primary"
                ...
                <Timeline
                  items={wf.steps.map((step, si) => ({
                    color: si < wf.steps.length - 1 ? D.primary : D.success,
                    children: <Text style={{ color: D.textSec, fontSize: 13 }}>{step}</Text>,
                  }))}
                />
              </motion.div>
            </Col>
      ...
      <Modal
        title={<Text style={{ fontWeight: 700 }}>审批详情</Text>}
        open={detailOpen}
          ...
          <div style={{ padding: '20px 24px' }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              ...
              <Tag
                style={{
                  ...
                  color: STATUS_MAP.pending.color,
                  border: 'none',
                  fontWeight: 600,
                }}
              >
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <Button
                ...
                style={{ flex: 1, borderRadius: 10, height: 42, color: D.danger }}
              >
                驳回
              </Button>
            </div>
