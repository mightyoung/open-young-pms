...
function GanttBar({ task, color }) {
  const start = new Date(
    task.start.includes('-') && !task.start.includes('T') ? `2026-${task.start}` : task.start
  )
  const end = new Date(
    task.end.includes('-') && !task.end.includes('T') ? `2026-${task.end}` : task.end
  )
...
export default function Gantt() {
  ...
  const STATUS_MAP = {
    ...
    inprogress: { label: '进行中', bg: '#dbeafe', color: '#1e40af' },
    pending: { label: '待开始', bg: '#f3f4f6', color: '#6b7280' },
  }
  ...
  const taskColumns = [
    ...
    },
    {
      ...
      key: 'wbs',
      render: v => (
        <Text style={{ fontFamily: 'monospace', fontSize: 12, color: D.textSec }}>{v}</Text>
      ),
    },
    {
      title: '负责人',
  ...
  return (
    <div>
      ...
      <Space style={{ marginBottom: 16 }}>
        {[
          ...
          <Button
            ...
            style={{
              borderRadius: 10,
              ...(tab === k ? { background: D.primary, border: 'none' } : {}),
            }}
          >
            {label}
          </Button>
        ...
        <motion.div variants={va(0)} initial="hidden" animate="visible">
          ...
          >
            <div style={{ marginBottom: 12 }}>
              ...
              <Text style={{ color: D.textMuted, fontSize: 12, marginLeft: 12 }}>
        ...
        <motion.div variants={va(0)} initial="hidden" animate="visible">
          ...
          >
            <Tree
              showLine
              defaultExpandAll
              treeData={MOCK_WBS.map(n => ({
                title: (
                  <Space>
                    <FolderOutlined style={{ color: D.primary }} />
                ...
                children: n.children?.map(c => ({
                  title: (
                    <Space>
                      <FolderOutlined style={{ color: D.warning }} />
                      <Text>{c.name}</Text>
                      <Tag
                        style={{
                          marginLeft: 8,
                          background: c.progress === 100 ? '#dcfce7' : '#dbeafe',
                          color: c.progress === 100 ? '#166534' : '#1e40af',
                          border: 'none',
                          fontSize: 11,
                        }}
                      >
                  ...
                  children: c.children?.map(s => ({
                    title: (
                      <Space>
                        <ClockCircleOutlined style={{ color: D.textMuted, fontSize: 12 }} />
                        <Text style={{ fontSize: 13 }}>{s.name}</Text>
                        <Tag
                          style={{
                            background: s.progress === 100 ? '#dcfce7' : '#dbeafe',
                            color: s.progress === 100 ? '#166534' : '#1e40af',
                            border: 'none',
                            fontSize: 11,
                          }}
                        >
                        ...
                        <Text style={{ color: D.textMuted, fontSize: 11 }}>{s.assign}</Text>
                      </Space>
                    ),
                    key: s.id,
                  })),
      ...
      <Modal
        title={<Text style={{ fontWeight: 700 }}>新建任务</Text>}
        open={modalOpen}
        onCancel={() => {
          setModalOpen(false)
          form.resetFields()
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="name" label="任务名称" rules={[{ required: true }]}>
                <Input placeholder="任务名称" style={{ borderRadius: 10 }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="assignee" label="负责人" rules={[{ required: true }]}>
                <Input placeholder="负责人" style={{ borderRadius: 10 }} />
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="wbs" label="WBS编号">
                <Input placeholder="如 1.3.1" style={{ borderRadius: 10 }} />
              </Form.Item>
            </Col>
            <Col span={12}>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="start" label="开始日期">
                <Input placeholder="04-01" style={{ borderRadius: 10 }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="end" label="结束日期">
                <Input placeholder="04-10" style={{ borderRadius: 10 }} />
              </Form.Item>
            </Col>
          </Row>
        </Form>
