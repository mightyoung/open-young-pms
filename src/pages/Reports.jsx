      ...
      >
        <div style={{ padding: '16px 24px', borderBottom: `1px solid ${D.border}` }}>
          <Steps
            current={step}
            size="small"
            items={[{ title: '选择项目' }, { title: '填写内容' }, { title: '提交审批' }]}
          />
        </div>
        <div style={{ padding: '20px 24px' }}>
          {step === 0 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              <Text style={{ fontWeight: 600, color: D.text, display: 'block', marginBottom: 16 }}>
                选择报告类型和项目
              </Text>
              <Form form={form} layout="vertical">
                ...
                <Form.Item name="project" label="所属项目" rules={[{ required: true }]}>
                  <Select
                    placeholder="选择项目"
                    options={[
                      { value: 'J-2X高精线联调项目', label: 'J-2X高精线联调项目' },
                      { value: 'O3厂区建设项目', label: 'O3厂区建设项目' },
                      { value: '高压管路安装工程', label: '高压管路安装工程' },
                    ]}
                    style={{ borderRadius: 10 }}
                  />
                </Form.Item>
              ...
              <Button
                ...
                block
                size="large"
                style={{ borderRadius: 12, background: D.primary, marginTop: 8 }}
                onClick={() => setStep(1)}
              >
                下一步
              </Button>
          ...
          {step === 1 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <Form form={form} layout="vertical">
                <Form.Item name="progress" label="本周/日本周工作进展" rules={[{ required: true }]}>
                  <TextArea
                    rows={4}
                    placeholder="请详细描述工作进展..."
                    style={{ borderRadius: 10 }}
                  />
                </Form.Item>
                <Form.Item name="completed" label="完成工作项（逗号分隔）">
                  <Input placeholder="如：主轴落位、接线检查" style={{ borderRadius: 10 }} />
                </Form.Item>
                <Form.Item name="pending" label="待推进事项（逗号分隔）">
                  <Input placeholder="如：等待图纸变更" style={{ borderRadius: 10 }} />
                </Form.Item>
                <Form.Item name="issues" label="问题与风险">
                  <TextArea
                    rows={2}
                    placeholder="如有重大问题请说明..."
                    style={{ borderRadius: 10 }}
                  />
                </Form.Item>
              </Form>
              <Space style={{ width: '100%', justifyContent: 'space-between' }}>
                <Button onClick={() => setStep(0)} style={{ borderRadius: 10 }}>
                  上一步
                </Button>
                <Button
                  type="primary"
        ...
        {selected && (
          <div style={{ padding: '20px 24px' }}>
            ...
            <Text style={{ color: D.textMuted, fontSize: 12 }}>
              {selected.project} · {selected.author} · {selected.createTime}
            </Text>
            <Divider style={{ margin: '16px 0' }} />
            <Title level={5} style={{ color: D.text, marginBottom: 8 }}>
              报告内容
            </Title>
            ...
            <Row gutter={[12, 12]} style={{ marginBottom: 16 }}>
              {[
                { label: '完成', value: selected.stats.completed, color: D.success },
                { label: '进行中', value: selected.stats.pending, color: D.warning },
                { label: '问题', value: selected.stats.issues, color: D.danger },
              ].map(s => (
            ...
            {selected.status === 'pending' && (
              ...
              >
                <Text style={{ color: D.warning, fontSize: 13 }}>
                  <ClockCircleOutlined /> 等待审批中
                </Text>
              </div>
            )}
          </div>
