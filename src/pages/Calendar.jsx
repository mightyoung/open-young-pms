        ...
        {cells.map((day, idx) => {
          ...
          return (
            ...
            >
              <Text
                style={{
                  color: isToday(day) ? colors.accent : colors.text.primary,
                  fontSize: 13,
              ...
              <div style={{ marginTop: 2 }}>
                {dayEvents.slice(0, 2).map((e, i) => (
                  ...
                  >
                    <Text
                      style={{
                        color: e.type === 'milestone' ? colors.warning : colors.accent,
                        fontSize: 10,
                      }}
                    >
                      {e.text}
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([proj, phases, tasks]) => {
        ...
        ;(phases || []).forEach(p => {
          if (p.start_date)
            evts.push({ date: p.start_date.substring(0, 10), text: p.name, type: 'phase' })
          if (p.end_date)
            evts.push({ date: p.end_date.substring(0, 10), text: p.name + '截止', type: 'phase' })
        })
        // 任务
        ;(tasks || []).forEach(t => {
          if (t.start_date)
            evts.push({
              date: t.start_date.substring(0, 10),
              text: t.name || t.title,
              type: 'task',
            })
        })
