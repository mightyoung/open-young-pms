...
export default function NotificationSettings() {
  ...
  useEffect(() => {
    api.get('/notification-settings').then(res => {
      ...
      <Card style={{ background: '#ffffff', border: '1px solid #e5e7eb', marginBottom: 16 }}>
        ...
        <div style={{ marginTop: 8 }}>
          <Select value={settings.channel} onChange={v => setSettings(s => ({ ...s, channel: v }))} style={{ width: 160 }}>
            <Select.Option value="in_app"><MessageOutlined /> 应用内通知</Select.Option>
            <Select.Option value="email"><MailOutlined /> 邮件通知</Select.Option>
            <Select.Option value="wechat"><WechatOutlined /> 微信通知</Select.Option>
          </Select>
        </div>
      </Card>
      ...
      <Card style={{ background: '#ffffff', border: '1px solid #e5e7eb' }}>
        ...
        {items.map(item => (
          <div key={item.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #e5e7eb' }}>
            ...
            <Switch checked={settings[item.key]} onChange={v => setSettings(s => ({ ...s, [item.key]: v }))} />
          </div>
        ))}
        <Button type="primary" style={{ marginTop: 16 }} onClick={handleSave} loading={saving}>保存设置</Button>
      </Card>
    </div>
  )
}
