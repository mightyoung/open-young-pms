...
const DEFAULT_RULES = [
  {
    id: 1,
    name: '紧急隐患自动派发',
    trigger: 'urgency=urgent AND type=safety',
    assignee: '科室负责人',
    enabled: true,
  },
...
export default function HazardRules() {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'trigger',
      render: t => (
        <Text style={{ color: colors.text.secondary, fontFamily: 'monospace', fontSize: 12 }}>
          {t}
        </Text>
      ),
    },
    { title: '自动派发给', dataIndex: 'assignee', render: t => <Tag color="blue">{t}</Tag> },
    {
      title: '状态',
      dataIndex: 'enabled',
      render: (v, r) => <Switch checked={v} onChange={() => toggleRule(r.id)} size="small" />,
    },
  ]
