...
export default function ReportManagementV2() {
  ...
  const stats = useMemo(
    () => ({
      ...
      avgQuality: Math.round(
        MOCK_REPORTS.reduce((acc, r) => acc + r.quality, 0) / MOCK_REPORTS.length
      ),
    }),
    []
  )
  ...
  const columns = [
      ...
      render: v => (
        <Space>
          <Progress
            ...
            size="small"
            style={{ width: 60 }}
            strokeColor={v >= 80 ? COLORS.success : v >= 60 ? COLORS.warning : COLORS.danger}
          />
    ...
    },
    {
      ...
      width: 90,
      render: v => (
        <Tag color={v === 'submitted' ? 'success' : 'default'}>
          {v === 'submitted' ? '已提交' : '草稿'}
        </Tag>
      ),
    },
    {
      ...
      render: (_, r) => (
        <Space>
          <Button size="small" icon={<Eye size={12} />} onClick={() => setPreviewReport(r)}>
            预览
          </Button>
  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: '100vh', padding: '24px 28px' }}>
      <Card
        ...
        extra={
          <Space>
            <Button icon={<ClipboardList size={14} />} onClick={() => setShowClipboardList(true)}>
              模板市场
            </Button>
            <Button type="primary" icon={<Zap size={14} />} onClick={() => setShowAutoModal(true)}>
              自动生成
            </Button>
          </Space>
        }
      ...
      >
        <Row gutter={16} style={{ marginBottom: 24 }}>
          {[
            ...
            {
              label: '平均质量',
              value: `${stats.avgQuality}分`,
              icon: <Star size={20} />,
              color: stats.avgQuality >= 75 ? COLORS.success : COLORS.danger,
            },
          ].map(s => (
                ...
                <List.Item
                  actions={[
                    <Button
                      ...
                      type="primary"
                      icon={<Zap size={12} />}
                      onClick={() => handleAutoGenerate(item.type)}
                    >
                      立即生成
                    </Button>,
                    <Button key="toggle" size="small" icon={item.enabled ? '暂停' : '启用'}>
                      {item.enabled ? '暂停' : '启用'}
                    </Button>,
                  ]}
                >
                  <List.Item.Meta
                    ...
                    description={
                      <Space>
                        <Clock size={12} />
                        <Text type="secondary">{item.cron}</Text>
                ...
                {[
                  ...
                  >
                    ...
                    >
                      ...
                      <Tag
                        color={
                          item.score >= 80 ? 'success' : item.score >= 60 ? 'warning' : 'error'
                        }
                      >
                        {item.score}分
                      </Tag>
      ...
      <Modal
        title="报告模板市场"
        open={showClipboardList}
        onCancel={() => setShowClipboardList(false)}
        footer={<Button onClick={() => setShowClipboardList(false)}>关闭</Button>}
        width={700}
      >
        <Row gutter={[16, 16]}>
            ...
            <Col span={12} key={t.id}>
              ...
              >
                ...
                <div style={{ marginTop: 12 }}>
                  <Button type="primary" size="small" icon={<FileText size={12} />} block>
                    使用模板
                  </Button>
                </div>
              </Card>
      ...
      >
        <Alert
          message="系统将根据已配置的报告模板和数据，自动生成报告初稿"
          type="info"
          style={{ marginBottom: 16 }}
        />
        <Row gutter={[12, 12]}>
          {REPORT_TEMPLATES.slice(0, 3).map(t => (
            <Col span={8} key={t.id}>
              <Card
                size="small"
                hoverable
                onClick={() => handleAutoGenerate(t.type)}
      ...
      <Modal
        ...
        footer={
          <Space>
            <Button onClick={() => setPreviewReport(null)}>关闭</Button>
        ...
        {previewReport && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              ...
              <Space>
                <Tag>
                  <User size={12} /> {previewReport.author}
                </Tag>
                <Tag>
                  <Calendar size={12} /> {previewReport.date}
                </Tag>
                <Tag
                  color={
                    previewReport.quality >= 80
                      ? 'success'
                      : previewReport.quality >= 60
                        ? 'warning'
                        : 'error'
                  }
                >
            ...
            >
              {generateReportContent(
                previewReport.type,
                { start: previewReport.date, end: previewReport.date },
                {
                  tasks: MOCK_REPORTS.filter(r => r.type === previewReport.type).map(r => ({
                    ...r,
                    status: 'done',
                  })),
                  hazards: MOCK_REPORTS.filter(r => r.type === 'safety'),
