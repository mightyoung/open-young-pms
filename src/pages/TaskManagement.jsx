...
import { Card, Table, Button, Tag, Space, Modal, Form, Input, Select, InputNumber, DatePicker, Row, Col, Typography, Progress, Badge, Tooltip, Tree, Divider, message } from 'antd';
import { Plus, Edit2, Trash2, CheckCircle2, Clock, User, Calendar, FolderTree, GanttChart, Filter, Export } from 'lucide-react';
...
const TASKS = [
  { id: 1, wbs: '1.1', title: '需求调研', phase: '方案设计', assignee: '张三', startDate: '2026-01-15', endDate: '2026-02-15', progress: 100, status: 'done', priority: 'high' },
  { id: 2, wbs: '1.2', title: '技术方案编制', phase: '方案设计', assignee: '李四', startDate: '2026-02-01', endDate: '2026-03-15', progress: 100, status: 'done', priority: 'high' },
  { id: 3, wbs: '1.3', title: '方案评审', phase: '方案设计', assignee: '王五', startDate: '2026-03-01', endDate: '2026-04-15', progress: 80, status: 'inprogress', priority: 'high' },
  { id: 4, wbs: '2.1', title: '设备选型', phase: '设备采购', assignee: '赵六', startDate: '2026-02-15', endDate: '2026-03-31', progress: 100, status: 'done', priority: 'medium' },
  { id: 5, wbs: '2.2', title: '招标', phase: '设备采购', assignee: '钱七', startDate: '2026-03-15', endDate: '2026-05-31', progress: 65, status: 'inprogress', priority: 'medium' },
  { id: 6, wbs: '2.3', title: '合同签订', phase: '设备采购', assignee: '孙八', startDate: '2026-05-01', endDate: '2026-06-30', progress: 30, status: 'inprogress', priority: 'low' },
];
...
export default function TaskManagement() {
  ...
  const columns = [
    ...
    { title: '任务名称', dataIndex: 'title', key: 'title', width: 150,
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          <Tooltip title="编辑"><Button type="text" size="small" icon={<Edit2 size={14} />} onClick={() => handleEditTask(record)} /></Tooltip>
          <Tooltip title="删除"><Button type="text" size="small" danger icon={<Trash2 size={14} />} onClick={() => handleDeleteTask(record)} /></Tooltip>
        </Space>
      ),
    },
  ];
  ...
  return (
    <div style={{ padding: 24, background: COLORS.bg, minHeight: '100vh' }}>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <Title level={3} style={{ color: COLORS.text, margin: 0 }}>任务管理</Title>
          <Text style={{ color: COLORS.textMuted }}>WBS分解 · 甘特图 · 进度追踪</Text>
        </div>
        <Space>
          <Button.Group>
            <Button icon={<FolderTree size={14} />} type={view === 'wbs' ? 'primary' : 'default'} onClick={() => setView('wbs')}>WBS视图</Button>
            <Button icon={<GanttChart size={14} />} type={view === 'gantt' ? 'primary' : 'default'} onClick={() => setView('gantt')}>甘特图</Button>
          </Button.Group>
          <Button icon={<Export size={14} />} onClick={() => message.info('导出功能开发中')}>导出</Button>
          <Button type="primary" icon={<Plus size={14} />} onClick={handleAdd}>新建任务</Button>
        </Space>
      </div>

      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={6}><Card style={{ borderRadius: 12, textAlign: 'center' }}>
          <div style={{ fontSize: 24, fontWeight: 'bold', color: COLORS.primary }}>{tasks.length}</div>
          <div style={{ fontSize: 12, color: COLORS.textMuted }}>总任务数</div>
        </Card></Col>
        <Col span={6}><Card style={{ borderRadius: 12, textAlign: 'center' }}>
          <div style={{ fontSize: 24, fontWeight: 'bold', color: COLORS.success }}>{tasks.filter(t => t.status === 'done').length}</div>
          <div style={{ fontSize: 12, color: COLORS.textMuted }}>已完成</div>
        </Card></Col>
        <Col span={6}><Card style={{ borderRadius: 12, textAlign: 'center' }}>
          <div style={{ fontSize: 24, fontWeight: 'bold', color: COLORS.warning }}>{tasks.filter(t => t.status === 'inprogress').length}</div>
                ...
                <Tree
                  ...
                  titleRender={(node) => (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '4px 0' }}>
                      <span style={{ fontFamily: 'monospace', color: COLORS.textMuted, fontSize: 12 }}>{node.wbs}</span>
            ...
            {view === 'gantt' && (
              <div>
                ...
                <div style={{ overflowX: 'auto' }}>
                  <div style={{ display: 'flex', marginLeft: 120, borderBottom: `1px solid ${COLORS.border}`, paddingBottom: 8, marginBottom: 16 }}>
                    {['1月', '2月', '3月', '4月', '5月', '6月'].map((m, i) => (
                      <div key={i} style={{ width: 100, textAlign: 'center', fontSize: 12, color: COLORS.textMuted }}>{m}</div>
                    ))}
                  </div>
                  {GANTT_DATA.map((task, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', marginBottom: 12 }}>
                      <div style={{ width: 120, fontSize: 12, color: COLORS.text }}>{task.name}</div>
                      <div style={{ flex: 1, height: 24, background: COLORS.bg, borderRadius: 4, position: 'relative' }}>
                        <div style={{
                          position: 'absolute',
                        ...
                        <span style={{ position: 'absolute', left: 8, top: 4, fontSize: 10, color: '#fff' }}>{task.progress}%</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <Card title="进度对比" style={{ borderRadius: 12 }}>
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={WEEKLY_PROGRESS}>
                  ...
                  <Line type="monotone" dataKey="planned" stroke={COLORS.primary} strokeDasharray="5 5" name="计划" />
      ...
      >
        <Form form={form} layout="vertical">
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="wbs" label="WBS编码"><Input placeholder="如 1.1.1" /></Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="title" label="任务名称" rules={[{ required: true, message: '请输入任务名称' }]}><Input /></Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="phase" label="所属阶段"><Select placeholder="选择阶段"><Select.Option value="方案设计">方案设计</Select.Option><Select.Option value="设备采购">设备采购</Select.Option><Select.Option value="安装施工">安装施工</Select.Option><Select.Option value="调试验收">调试验收</Select.Option></Select></Form.Item>
            </Col>
            <Col span={12}>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="date" label="计划时间" rules={[{ required: true, message: '请选择时间' }]}><RangePicker style={{ width: '100%' }} /></Form.Item>
            </Col>
            <Col span={12}>
