  ...
  const columns = [
    {
      title: '费用科目',
      dataIndex: 'category',
      render: t => <Text style={{ color: colors.text.primary }}>{t}</Text>,
    },
    {
      title: '预算金额',
      dataIndex: 'budget',
    ...
    },
    {
      ...
      render: (v, r) => (
        <div>
          ...
          <Progress
            percent={r.budget > 0 ? Math.round((v / r.budget) * 100) : 0}
            size="small"
    ...
    },
    {
      ...
      render: (_, r) => {
        ...
        return (
          <Text
            style={{
              color: pct > 100 ? colors.danger : pct > 80 ? colors.warning : colors.success,
            }}
          >
            {pct}%
          </Text>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={6}>
          <Card
            style={{ background: colors.bg.page, border: '1px solid #e5e7eb', textAlign: 'center' }}
          >
            <Statistic
              title={<Text style={{ color: colors.text.muted }}>总预算</Text>}
              value={totalBudget}
              valueStyle={{ color: colors.accent, fontSize: 20 }}
              suffix="元"
            />
          </Card>
        </Col>
        <Col span={6}>
          ...
          >
            <Statistic
              title={<Text style={{ color: colors.text.muted }}>已执行</Text>}
              value={totalSpent}
        ...
        <Col span={6}>
          ...
          >
            <Statistic
              title={<Text style={{ color: colors.text.muted }}>执行率</Text>}
              value={executionRate}
              valueStyle={{
                color:
                  executionRate > 100
              ...
              suffix="%"
              prefix={executionRate > 100 ? <WarningOutlined /> : <CheckCircleOutlined />}
            />
          </Card>
        </Col>
        <Col span={6}>
          ...
          >
            <Statistic
              title={<Text style={{ color: colors.text.muted }}>剩余预算</Text>}
              value={Math.max(0, totalBudget - totalSpent)}
              valueStyle={{ color: colors.text.muted, fontSize: 20 }}
              suffix="元"
            />
          </Card>
        </Col>
