...
import { Card, Tag, Button, Select, Space, Modal, Form, Input, InputNumber, Upload, message, Row, Col, Typography, Avatar, Tabs, Divider, Tooltip, Timeline } from 'antd'
import { PlusOutlined, CameraOutlined, EnvironmentOutlined, SendOutlined, LoadingOutlined, DeleteOutlined, PictureOutlined, CheckCircleOutlined, ClockCircleOutlined, ExclamationCircleOutlined } from '@ant-design/icons'
...
const MOCK_DATA = [
  { id: 1, title: '3号车间配电箱门未关闭', type: 'safety', level: 'urgent', status: 'rectifying', location: '3号车间-A区配电房', reporter: '李师傅', assignee: '王安全', phone: '138****1234', desc: '配电箱门敞开，存在触电风险，现场已设置警示标志。', photos: 2, gps: '31.2304°N, 121.4737°E', createTime: '10:20', deadline: '12:20' },
  { id: 2, title: '钢结构焊接质量不达标', type: 'quality', level: 'major', status: 'pending_acceptance', location: '2号厂房-北侧钢结构', reporter: '张监', assignee: '陈工', phone: '139****5678', desc: '焊缝存在气孔，未按图施工，需整改后重新验收。', photos: 3, gps: '31.2310°N, 121.4740°E', createTime: '昨天 14:30', deadline: '今天 14:30' },
  { id: 3, title: '施工废料未及时清理', type: 'environment', level: 'general', status: 'assigned', location: 'O3厂区-东侧施工区', reporter: '刘工', assignee: '李师傅', phone: '137****9012', desc: '现场堆积大量废钢材和包装材料，影响通道。', photos: 1, gps: '31.2295°N, 121.4730°E', createTime: '昨天 09:15', deadline: '明天 09:15' },
  { id: 4, title: '塔吊电缆绝缘层破损', type: 'equipment', level: 'urgent', status: 'pending', location: '主厂房-3号塔吊', reporter: '王师傅', assignee: '待分配', phone: '136****3456', desc: '塔吊主电缆外护套破损，需立即停电检修。', photos: 2, gps: '31.2308°N, 121.4735°E', createTime: '今天 08:05', deadline: '10:05' },
  { id: 5, title: '临边防护栏杆缺失', type: 'safety', level: 'major', status: 'confirmed', location: '2号平台-3层临边', reporter: '李师傅', assignee: '张工', phone: '138****7890', desc: '2号平台3层临边防护栏杆缺失，已用临时围栏隔离。', photos: 1, gps: '31.2302°N, 121.4742°E', createTime: '前天 16:40', deadline: '昨天 16:40' },
  { id: 6, title: '脚手架扣件松动', type: 'safety', level: 'general', status: 'closed', location: '3号车间-外脚手架', reporter: '王工', assignee: '李师傅', phone: '139****2345', desc: '外脚手架部分扣件松动，整改完成，已验收。', photos: 2, gps: '31.2298°N, 121.4728°E', createTime: '3天前', deadline: '—' },
]

function va(i = 0) { return { hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0, transition: { delay: i * 0.06, duration: 0.3 } } } }
...
export default function Hazards() {
  ...
  const getGps = () => {
    setGpsLoading(true)
    if (!navigator.geolocation) { message.error('浏览器不支持定位'); setGpsLoading(false); return }
    navigator.geolocation.getCurrentPosition(
      (pos) => { setGps(`${pos.coords.latitude.toFixed(4)}°N, ${pos.coords.longitude.toFixed(4)}°E`); setGpsLoading(false); message.success('定位成功') },
      () => { setGps('31.2304°N, 121.4737°E（默认）'); setGpsLoading(false); message.warning('定位失败，使用默认位置') },
  ...
  const autoDispatch = (type, level) => {
    ...
    if (rule) { setDispatchResult(rule); return rule }
  ...
  const handleReport = async () => {
    try {
      ...
      message.success(`上报成功${rule ? `，已自动派发给 ${rule.assignee}（${rule.dept}），SLA ${rule.sla}` : '，请手动指派整改人'}`)
      setReportOpen(false)
      setSubmitting(false)
      form.resetFields()
      setPhotos([])
  ...
  return (
    <div>
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 20 }}>
        ...
        ].map((s, i) => <Col xs={12} sm={6} key={s.title}><motion.div variants={va(i)} initial="hidden" animate="visible"><StatCard {...s} /></motion.div></Col>)}
      </Row>
      ...
      <Card style={{ border: `1px solid ${D.border}`, borderRadius: 14 }} headStyle={{ borderBottom: `1px solid ${D.border}`, padding: '12px 20px' }} bodyStyle={{ padding: '16px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
          ...
          <Button type="primary" icon={<PlusOutlined />} style={{ borderRadius: 10, background: D.primary }} onClick={() => { form.resetFields(); setPhotos([]); setGps(null); setDispatchResult(null); setReportOpen(true) }}>
            上报隐患
          </Button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <AnimatePresence>
            {filtered.map((item, i) => {
              ...
              return (
                <motion.div key={item.id} variants={va(i)} initial="hidden" animate="visible" exit={{ opacity: 0, scale: 0.97 }}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                        ...
                        {item.photos > 0 && <Tag style={{ background: `${D.primary}15`, color: D.primary, border: 'none', fontSize: 12 }}><CameraOutlined /> {item.photos}张</Tag>}
                      </div>
                      <Text style={{ fontWeight: 600, fontSize: 14, color: D.text, display: 'block', marginBottom: 6 }}>{item.title}</Text>
                      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                        <Text style={{ color: D.textMuted, fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}><EnvironmentOutlined /> {item.location}</Text>
          ...
          {filtered.length === 0 && <div style={{ textAlign: 'center', padding: 60, color: D.textMuted }}>暂无数据</div>}
        </div>
      </Card>

      <Modal
        title={<Text style={{ fontWeight: 700, fontSize: 16 }}>上报隐患</Text>}
        open={reportOpen} onCancel={() => setReportOpen(false)} footer={null}
        width={620}
        styles={{ content: { borderRadius: 16, padding: 0 }, header: { borderBottom: `1px solid ${D.border}`, padding: '16px 24px', margin: 0 } }}
      >
        <Form form={form} layout="vertical" style={{ padding: '20px 24px' }} onValuesChange={(c) => { if (c.type || c.level) { const t = form.getFieldValue('type'); const l = form.getFieldValue('level'); if (t && l) autoDispatch(t, l) } }}>
          <Row gutter={16}>
            <Col span={12}><Form.Item name="type" label={<Text style={{ fontSize: 13 }}>问题类型</Text>} rules={[{ required: true }]}>
              <Select placeholder="选择类型" options={Object.entries(TYPE_MAP).map(([k, v]) => ({ value: k, label: v.label }))} style={{ borderRadius: 10 }} />
            </Form.Item></Col>
            <Col span={12}><Form.Item name="level" label={<Text style={{ fontSize: 13 }}>紧急程度</Text>} rules={[{ required: true }]}>
              <Select placeholder="选择程度" options={Object.entries(LEVEL_MAP).map(([k, v]) => ({ value: k, label: v.label }))} style={{ borderRadius: 10 }} />
            </Form.Item></Col>
          </Row>
          <Form.Item name="title" label={<Text style={{ fontSize: 13 }}>问题标题</Text>} rules={[{ required: true, message: '请输入问题标题' }]}><Input placeholder="简要描述问题" style={{ borderRadius: 10 }} /></Form.Item>
          <Form.Item name="location" label={<Text style={{ fontSize: 13 }}>问题位置</Text>} rules={[{ required: true }]}><Input placeholder="如：3号车间-A区配电房" style={{ borderRadius: 10 }} addonBefore={<EnvironmentOutlined style={{ color: D.primary }} />} /></Form.Item>
          <Form.Item name="description" label={<Text style={{ fontSize: 13 }}>问题描述</Text>}><TextArea rows={3} placeholder="详细描述问题情况..." style={{ borderRadius: 10 }} /></Form.Item>
          ...
          {dispatchResult && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <Text style={{ color: D.textSec, fontSize: 13 }}>派发给：<Text style={{ fontWeight: 600, color: D.text }}>{dispatchResult.assignee}</Text>（{dispatchResult.dept}），SLA：{dispatchResult.sla}</Text>
            </motion.div>
          )}

          <Button type="primary" block size="large" icon={<SendOutlined />} loading={submitting} onClick={handleReport}
            style={{ borderRadius: 12, background: D.primary, height: 48, fontSize: 15 }}>提交上报</Button>
        </Form>
      </Modal>
