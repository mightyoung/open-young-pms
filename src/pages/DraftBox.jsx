...
export default function DraftBox() {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {loading ? (
        <SkeletonContent type="cards" count={3} />
      ) : drafts.length === 0 ? (
        ...
        <List
          dataSource={drafts}
          renderItem={item => {
            const cfg = STATUS_MAP[item.hazard_type] || {
              label: item.hazard_type,
              color: 'default',
            }
            return (
              <List.Item
                actions={[
                  <Button
                    ...
                    size="small"
                    icon={<EditOutlined />}
                    onClick={() => (window.location.hash = `#/hazards/report?draft=${item.id}`)}
                  >
                    继续编辑
                  </Button>,
                  <Popconfirm
                  ...
                  description={
                    <div style={{ color: colors.text.muted, fontSize: 12 }}>
                      ...
                      <span style={{ float: 'right' }}>
                        保存于 {formatTime(item.auto_saved_at || item.created_at)}
                      </span>
                    </div>
                  }
                />
