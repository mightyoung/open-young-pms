...
const DICT_ITEMS = [
  {
    ...
    name: '项目类型',
    category: '项目管理',
    items: '产线改造,设备采购安装,厂房建设,技术研发,其他',
    count: 5,
  },
  {
    id: 2,
    ...
    name: '隐患类型',
    category: '安全管理',
    items: '安全隐患,质量缺陷,设备故障,工艺偏差,环境问题,进度异常',
    count: 6,
  },
  {
    id: 3,
                ...
                <Row gutter={16} style={{ marginBottom: 16 }}>
                  ...
                  <Col span={6}>
                    <Card style={{ borderRadius: 12, textAlign: 'center' }}>
                      <Statistic
                        title="整体质量评分"
                        value={DATA_QUALITY.overall}
                        suffix="/100"
                        valueStyle={{
                          color: DATA_QUALITY.overall > 90 ? COLORS.success : COLORS.warning,
                        }}
                      />
                    </Card>
                  </Col>
                ...
                <Row gutter={16}>
                  <Col span={12}>
                    <Card title="数据质量雷达" style={{ borderRadius: 12 }}>
                      ...
                      <div
                        style={{ display: 'flex', justifyContent: 'space-around', marginTop: 16 }}
                      >
                        {[
                    ...
                    <Card title="数据安全等级分布" style={{ borderRadius: 12 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                        {SECURITY_LEVELS.map((level, i) => {
                          const count = ASSET_LIST.filter(
                            a => a.securityLevel === level.value
                          ).length
                          return (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                              ...
                              <Progress
                                percent={(count / ASSET_LIST.length) * 100}
                                size="small"
                                strokeColor={level.color}
                                style={{ flex: 1 }}
                              />
                              <span style={{ fontSize: 12, color: COLORS.textMuted, width: 40 }}>
                                {count}个
                              </span>
                            </div>
                          )
                      ...
                      >
                        ...
                        >
                          ...
                          <Badge
                            ...
                            text={item.status === 'active' ? '运行中' : '已停用'}
                          />
                        </div>
