...
import { Card, Table, Tag, Button, Select, Form, Input, Modal, Space, message, Row, Col, Typography, Tabs } from 'antd'
import { PlusOutlined, CheckCircleOutlined, CloseCircleOutlined, FileTextOutlined, CheckOutlined, EditOutlined } from '@ant-design/icons'
...
function va(i = 0) {
  return { hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0, transition: { delay: i * 0.06 } } }
}
...
export default function QualityPage() {
  ...
  const stdColumns = [
    ...
    { title: '类别', dataIndex: 'category', key: 'category', render: v => <Tag style={{ background: CATEGORY_MAP[v]?.bg, color: CATEGORY_MAP[v]?.color, border: 'none', fontWeight: 600 }}>{CATEGORY_MAP[v]?.label}</Tag> },
  ...
  const insColumns = [
    ...
    { title: '检查名称', dataIndex: 'name', key: 'name', render: (v, r) => <div><Text style={{ fontWeight: 600 }}>{v}</Text><div><Text style={{ color: D.textMuted, fontSize: 12 }}>{r.project}</Text></div></div> },
    { title: '类别', dataIndex: 'category', key: 'category', render: v => <Tag style={{ background: CATEGORY_MAP[v]?.bg, color: CATEGORY_MAP[v]?.color, border: 'none', fontWeight: 600 }}>{CATEGORY_MAP[v]?.label}</Tag> },
    { title: '结果', dataIndex: 'result', key: 'result', render: v => {
      const c = RESULT_MAP[v]
      return <Tag style={{ background: c.bg, color: c.color, border: 'none', fontWeight: 600 }}>
        {v === 'passed' ? <CheckCircleOutlined /> : v === 'failed' ? <CloseCircleOutlined /> : null}{c.label}
      </Tag>
    }},
  ...
  return (
    <div>
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 20 }}>
        {[
          { title: '检查标准', value: standards.filter(s => s.status === 'active').length, color: D.primary },
          ...
          <Col xs={12} sm={6} key={s.title}>
            <motion.div variants={va(i)} initial="hidden" animate="visible" style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 12, padding: '14px 18px' }}>
      ...
      {tab === 'standards' && (
        <Card style={{ border: `1px solid ${D.border}`, borderRadius: 14 }} headStyle={{ borderBottom: `1px solid ${D.border}`, padding: '12px 20px' }} bodyStyle={{ padding: 0 }}>
          ...
          <Table columns={stdColumns} dataSource={standards} rowKey="id" pagination={{ pageSize: 8, showSizeChanger: false }} />
        </Card>
      )}

      {tab === 'inspections' && (
        <Card style={{ border: `1px solid ${D.border}`, borderRadius: 14 }} bodyStyle={{ padding: 0 }}>
          <Table columns={insColumns} dataSource={inspections} rowKey="id" pagination={{ pageSize: 8, showSizeChanger: false }} />
        </Card>
      )}

      <Modal title={<Text style={{ fontWeight: 700 }}>新建质量标准</Text>} open={modalOpen}
        onCancel={() => { setModalOpen(false); form.resetFields() }} onOk={handleCreate} okText="保存"
        okButtonProps={{ style: { background: D.primary, borderRadius: 10 } }} width={580}
      ...
      <Modal title={<Text style={{ fontWeight: 700 }}>标准详情</Text>} open={detailOpen}
        onCancel={() => setDetailOpen(false)} footer={null} width={580}
        styles={{ content: { borderRadius: 16, padding: 0 }, header: { borderBottom: `1px solid ${D.border}`, padding: '16px 24px', margin: 0 } }}>
        {selected && (
