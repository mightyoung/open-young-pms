...
def _task_to_response(task: WBSTask, children: list = None) -> TaskResponse:
    ...
    if task.assignee:
        assignee = UserBrief(
            id=str(task.assignee.id), username=task.assignee.username, full_name=task.assignee.full_name
        )
    return TaskResponse(
        id=str(task.id),
        title=task.title,
...
async def get_gantt_data(
    ...
    return [
        GanttTask(
            ...
            progress=t.progress,
            status=t.status,
            assignee=UserBrief(id=str(t.assignee.id), username=t.assignee.username, full_name=t.assignee.full_name)
            if t.assignee
            else None,
...
async def get_kanban_board(
    ...
    return KanbanBoard(
        columns={status: {"tasks": tasks_list, "count": len(tasks_list)} for status, tasks_list in columns.items()}
    )
...
async def get_critical_path(
    ...
    result = await db.execute(select(WBSTask).where(WBSTask.project_id == project_id).order_by(WBSTask.planned_start))
    tasks = result.scalars().all()
...
async def get_comments(
    ...
    result = await db.execute(
        select(WBSTaskComment).where(WBSTaskComment.task_id == task_id).order_by(WBSTaskComment.created_at)
    )
