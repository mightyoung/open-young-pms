...
async def start_approval(
    ...
    return ApiResponse.ok(
        {"instance_id": instance.id, "status": instance.status, "current_node": first_approver.get("name")}
    )


@router.get("/my-tasks")
async def my_tasks(
    status: str = None, page: int = 1, page_size: int = 20, db=Depends(get_db), current_user=Depends(get_current_user)
):
    ...
    total = (
        await db.execute(select(func.count(ApprovalTask.id)).where(ApprovalTask.assignee_id == user_id))
    ).scalar() or 0
    ...
    for task in rows:
        inst = (
            await db.execute(select(ApprovalInstance).where(ApprovalInstance.id == task.instance_id))
        ).scalar_one_or_none()
        _flow = None
        if inst:
        ...
        items.append(
            {
                ...
                "title": inst.variables.get("title", inst.entity_type)
                if inst and inst.variables
                else (inst.entity_type if inst else None),
...
async def approve_task(
    task_id: str, agree: bool = True, comment: str = None, db=Depends(get_db), current_user=Depends(get_current_user)
):
