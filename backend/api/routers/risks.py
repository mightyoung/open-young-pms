...
def _risk_dict(r):
    return dict(
        id=str(r.id), project_id=r.project_id, title=r.title,
...
async def list_risks(
    project_id: str = None, level: str = None, status: str = None,
    page: int = 1, page_size: int = 20,
    db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user),
):
    """风险列表"""
    query = select(Risk)
    if project_id: query = query.where(Risk.project_id == project_id)
    if level: query = query.where(Risk.level == level)
...
async def update_risk(risk_id: str, data: RiskUpdate, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
...
def _res_dict(r):
    return dict(
        id=str(r.id), project_id=r.project_id, name=r.name, category=r.category,
        spec=r.spec, quantity=r.quantity, unit=r.unit,
        status=r.status, supplier=r.supplier,
        order_date=r.order_date.isoformat() if r.order_date else None,
        delivery_date=r.delivery_date.isoformat() if r.delivery_date else None,
        test_result=r.test_result, remarks=r.remarks,
        created_at=r.created_at.isoformat() if r.created_at else None,
    )
...
async def list_resources(
    project_id: str = None, category: str = None, status: str = None,
    page: int = 1, page_size: int = 20,
    db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user),
):
    query = select(ResourceItem)
    if project_id: query = query.where(ResourceItem.project_id == project_id)
    if category: query = query.where(ResourceItem.category == category)
    if status: query = query.where(ResourceItem.status == status)
...
async def create_resource(data: ResourceItemCreate, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
...
async def update_resource(res_id: str, data: ResourceItemUpdate, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
    result = await db.execute(select(ResourceItem).where(ResourceItem.id == res_id))
    r = result.scalar_one_or_none()
    if not r: return ApiResponse.error("RS0001", "资源不存在")
    for key, val in data.model_dump(exclude_unset=True).items():
        setattr(r, key, val)
    await db.commit()
    return ApiResponse.ok({"id": str(r.id)})
