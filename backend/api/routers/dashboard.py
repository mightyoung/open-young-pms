...
async def dashboard_summary(period: str = "month", db=Depends(get_db), current_user=Depends(get_current_user)):
    ...
    hazard_recent = (
        await db.execute(select(func.count(HazardReport.id)).where(HazardReport.created_at >= start))
    ).scalar() or 0
    hazard_closed = (
        await db.execute(
            select(func.count(HazardReport.id)).where(HazardReport.status == "closed", HazardReport.updated_at >= start)
        )
    ).scalar() or 0
        ...
        cnt = (
            await db.execute(
                select(func.count(HazardReport.id)).where(
                    and_(HazardReport.created_at >= d0, HazardReport.created_at < d1)
                )
            )
        ).scalar() or 0
...
async def hazard_by_type(db=Depends(get_db), current_user=Depends(get_current_user)):
    rows = (
        await db.execute(
            select(HazardReport.hazard_type, func.count(HazardReport.id)).group_by(HazardReport.hazard_type)
        )
    ).all()
    total = sum(r[1] for r in rows)
    items = [
        {"name": r[0] or "其他", "value": r[1], "rate": round(r[1] / total * 100, 1) if total > 0 else 0} for r in rows
    ]
...
async def hazard_by_status(db=Depends(get_db), current_user=Depends(get_current_user)):
    ...
    rows = (
        await db.execute(select(HazardReport.status, func.count(HazardReport.id)).group_by(HazardReport.status))
    ).all()
    total = sum(r[1] for r in rows)
    items = [
        {"name": NAMES.get(r[0], r[0]), "value": r[1], "rate": round(r[1] / total * 100, 1) if total > 0 else 0}
        for r in rows
    ]
    return ApiResponse.ok({"items": items, "total": total})
        ...
        cnt = (
            await db.execute(
                select(func.count(Task.id)).where(
                    and_(Task.status == "done", Task.updated_at >= d0, Task.updated_at < d1)
                )
            )
        ).scalar() or 0
