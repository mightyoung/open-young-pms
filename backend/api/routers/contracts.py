...
async def list_contracts(
    ...
    items = [
        dict(
            ...
            amount=r.amount,
            status=r.status,
            signed_date=r.sign_date.isoformat() if r.sign_date else None,
            payment_terms=[],
            progress=100 if r.status in ("executed", "terminated") else 50 if r.status == "signed" else 0,
            payment=0,
    ...
    return ApiResponse.ok(
        {
            ...
            "amount": c.amount,
            "status": c.status,
            "signed_date": c.sign_date.isoformat() if c.sign_date else None,
            "start_date": c.start_date.isoformat() if c.start_date else None,
            "end_date": c.end_date.isoformat() if c.end_date else None,
            "payment_terms": [],
            "progress": 100 if c.status in ("executed", "terminated") else 50 if c.status == "signed" else 0,
            "payment": 0,
        }
    )
