...
async def list_points(keyword: str = None, db=Depends(get_db), current_user=Depends(get_current_user)):
    ...
    items = [
        {"id": r.id, "name": r.name, "code": r.qr_code, "location": r.location, "frequency": r.frequency} for r in rows
    ]
...
async def list_records(
    point_id: str = None, page: int = 1, page_size: int = 20, db=Depends(get_db), current_user=Depends(get_current_user)
):
    ...
    items = [
        {
            ...
            "point_id": r.point_id,
            "result": r.result,
            "result_name": RESULT_NAMES.get(r.result, r.result),
            "notes": r.notes,
            "photos": r.photos or [],
...
async def create_record(
    ...
    record = Inspection(point_id=point_id, inspector_id=str(current_user.id), result=result, notes=notes, photos=photos)
    db.add(record)
    await db.commit()
...
async def inspection_stats(db=Depends(get_db), current_user=Depends(get_current_user)):
    ...
    today_cnt = (
        await db.execute(select(func.count(Inspection.id)).where(Inspection.inspected_at >= today))
    ).scalar() or 0
    abnormal = (
        await db.execute(select(func.count(Inspection.id)).where(Inspection.result == "issue_found"))
    ).scalar() or 0
    return ApiResponse.ok(
        {
            "total_points": total,
            "today_checked": today_cnt,
            "total_abnormal": abnormal,
            "abnormal_rate": round(abnormal / (today_cnt or 1) * 100, 1),
        }
    )
