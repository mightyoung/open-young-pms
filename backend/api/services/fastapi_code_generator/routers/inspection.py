    ...
    return ApiResponse.ok(
        {
            ...
            "check_items": point.check_items or [],
        }
    )
...
async def create_point(
    ...
    point = InspectionPointV2(
        name=name, code=code, location=location, latitude=latitude, longitude=longitude, check_items=check_items
    )
    db.add(point)
    await db.commit()
...
async def inspection_stats(
    ...
    today_q = select(func.count(InspectionRecordV2.id)).where(InspectionRecordV2.checked_at >= today)
    today_count = (await db.execute(today_q)).scalar() or 0

    abnormal_q = select(func.count(InspectionRecordV2.id)).where(InspectionRecordV2.status == "abnormal")
