...
async def dashboard_summary(
    ...
    hazard_recent_q = select(func.count(HazardReport.id)).where(HazardReport.created_at >= start_date)
    hazard_recent = (await db.execute(hazard_recent_q)).scalar() or 0

    hazard_closed_q = select(func.count(HazardReport.id)).where(
        HazardReport.status == "closed", HazardReport.updated_at >= start_date
    )
...
async def hazard_trend(
    ...
    for i in range(days - 1, -1, -1):
        ...
        cnt_q = select(func.count(HazardReport.id)).where(
            and_(HazardReport.created_at >= day_start, HazardReport.created_at < day_end)
        )
...
async def hazard_by_type(
    ...
    """随手拍类型分布"""
    types_q = select(HazardReport.hazard_type, func.count(HazardReport.id)).group_by(HazardReport.hazard_type)
...
async def hazard_by_status(
    ...
    """随手拍状态分布"""
    status_q = select(HazardReport.status, func.count(HazardReport.id)).group_by(HazardReport.status)
    ...
    total = sum(r[1] for r in rows)
    items = [
        {"name": STATUS_NAMES.get(r[0], r[0]), "value": r[1], "rate": round(r[1] / total * 100, 1) if total > 0 else 0}
        for r in rows
    ]
    return ApiResponse.ok({"items": items, "total": total})
...
async def task_trend(
    ...
    for i in range(days - 1, -1, -1):
        ...
        done_q = select(func.count(Task.id)).where(
            and_(Task.status == "done", Task.updated_at >= day_start, Task.updated_at < day_end)
        )
